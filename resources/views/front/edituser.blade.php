...
<ul class="nav nav-tabs tabs-left profile-usermenu">
    <li><a href="{{route('userProfile')}}">@lang('profile.overview')</a></li>
...
<div class="profile-content">
    <form class="form" action="{{route('updateUserInfo')}}" method="post">
                            @csrf
    <table class="table table-user-information">
                    <tbody>
                            ...
                            <input type="hidden" name="userid" class="col-md-12 input-box" value="{{Auth::user()->id}}">
                            <tr>
                                <td>@lang('profile.full_name') :</td>
                                <td>
                                    <input type="text" name="name" class="col-md-12 input-box" value="{{Auth::user()->name}}" pattern="[A-Za-z\s]+" title="Enter only letters"  required>
                                </td>
                            </tr>
                            <tr>
                                <td>@lang('profile.dob') :</td>
                                <td>
                                    <input id="txtdate" type="text" name="dob" class="col-md-12 input-box" placeholder="Enter Date of birth" @if($userinfo) value="{{$userinfo->dob}}" @endif required>
                                </td>
                            </tr>
                            <td>@lang('profile.ph_no')</td>
                            <td>
                                <input type="number" id="phone_no" name="phone" class="col-md-12 input-box" placeholder="Enter Phone no"  @if($userinfo) value="{{$userinfo->phone}}" @endif required>
                            </td>
                            </tr>
                            <tr>
                            <td>@lang('profile.email')</td>
                               <td>
                                <input type="email" name="email" class="col-md-12 input-box" value="{{Auth::user()->email}}" readonly> 
                               </td>
                            </tr>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <textarea name="address" class="input-box col-md-12" cols="" rows="3" required>@if($userinfo) {!!$userinfo->address!!}@endif</textarea>
                                    </td>
                                    <td></td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                    <select name="language" class="input-box col-md-12" required>
                                        @php
                                        if($userinfo){
                                ...
                                <tr>
                                        ...
                                        <td> 
                                        <select name="currency" class="input-box col-md-12" required>
                                            
                                            <option value="USA Dollar" @if($currency == "USA Dollar") selected @endif>USA Dollar</option>
                                            <option value="CAD"  @if($currency == "CAD") selected @endif>CAD</option>
                                            <option value="UK Pound"  @if($currency == "UK Pound") selected @endif>UK Pound</option>
                                            <option value="Singapore Dollar"  @if($currency == "Singapore Dollar") selected @endif>Singapore Dollar</option>
                                        </select>
                                        </td>
                                    </tr>
                              ...
                              @if($userinfo)
                                <option value="{{$country->name}}" @if(($country->name) == ($userinfo->country)) selected="selected" @endif>{{$country->name}}</option>    @else
            ...
            <button class="btn btn-success pull-right btn-sm" style="margin-left: 3px"><a href="{{route('userProfile')}}" style="color: white;">@lang('profile.cancel')</a></button>
               <button type="submit" class="btn btn-success pull-right btn-sm">@lang('profile.save')</button>
             </form>     
</div>
</div>
...
<div class="tab-pane" id="saved_accommodation">
 @if($count_fav_accomodation !=0 && $accommodations !=0)
<div class="profile-content">
...
<div class="table-responsive table-striped">
                                    <table class="table">
                                       ...
                                       @foreach($accobookings as $accobooking)
                                        <tr>
                                            <td>{{$loop->iteration}}</td>
                                            <td><a href="{{route('accommodationDetail',['slug'=>$accobooking->slug,'id'=>$accobooking->accommodationId])}}">@if ( Config::get('app.locale') == 'en')
                          {{$accobooking->name}}
                        @elseif ( Config::get('app.locale') == 'tr' )
                          {{$accobooking->name_tr}}
                        @elseif ( Config::get('app.locale') == 'ar' )
...
<!-- profile courses content finish -->
</div>
</div>
</div>
<div class="clearfix"></div>
